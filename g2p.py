...
def load_cmudict():
    ...
    dict_ref = "text/cmudict-0.7b.txt"
    with open(dict_ref, encoding="ISO-8859-1") as fi:
        cmudict = (line.strip().split("  ") for line in islice(fi, 126, 133905))
        cmudict = {
            format_alt_entry(word): pronunciation.split() for word, pronunciation in cmudict
        }
    return cmudict
